...
def run(atoms_alpha, atoms_beta, disp, binned_data, extra_data, F, p, s):
    ...
    if use_interpolated_weights == True:
        ...
        extra_data['cu'] = np.concatenate( (extra_data['cu'], [0,0]) )
    ...
    old_minibin_index = exi.get_minibin_index(old_mu, s, root_index)
    P_product = 1

    # Open files for edge dmu's
    exi_output_files = exi.file_names('output', p, s)
    l_edge_dmu = open(exi_output_files['led'], 'a')
    r_edge_dmu = open(exi_output_files['red'], 'a')
    ...
    while FLAT < F or step <= Nsteps: # Flat condition only relevant for Wang Landau
        ...
        if new_mu < global_mu_min or new_mu > global_mu_max:
            ...
            # Update extra binned data. Note that, though misleading, by setting
            # accepted=True + expnt=0 -> P_product is multiplied by 1 as required
            cuts_counter, cuts_new_mu, cutold_s_mu, \
            minimat_counter, root_index, old_minibin_index, P_product \
                             = exi.update_binned(extra_data, mu_bins, old_mu, s,
                                    cuts_counter, cuts_new_mu, cutold_s_mu, 
                        old_index, True, dmu, abs(delta_local_energies[ACT]), 
                        minimat_counter, root_index, old_minibin_index, P_product, 0)
        ...
        # -------------------------- #
        #  Update extra binned data  #
        # -------------------------- #
        cuts_counter, cuts_new_mu, cutold_s_mu, \
        minimat_counter, root_index, old_minibin_index, P_product \
                         = exi.update_binned(extra_data, mu_bins, old_mu, s,
                                cuts_counter, cuts_new_mu, cutold_s_mu, 
                    old_index, accepted, dmu, abs(delta_local_energies[ACT]), 
